  ...
  const filteredData = useMemo(() => {
    ...
    Object.entries(structure).forEach(([key, value]) => {
      if (typeof value === "string") {
        ...
        const isFileEntry = entries.length > 0 && entries.every(([k, v]) => 
          typeof v === "string" && k.startsWith("/")
        );
          ...
          <div key={`dir-${dirName}`} className="select-none">
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1">
                {isExpanded ? (
                  <FaFolderOpen
                    className="text-blue-400 flex-shrink-0 w-3.5 h-3.5 
                ...
                <span
                  className="text-zinc-200 truncate font-medium group-hover:text-zinc-50 
                               transition-colors"
                  title={dirName}
                >
                  {dirName}
            ...
            {isExpanded && (
              <div
                className="ml-4 mt-1 border-l-2 border-zinc-800/50 pl-3 space-y-0.5 
      ...
      {sortedFileNames.map((fileName) => {
        ...
        return (
          ...
          >
            ...
            {!isEllipsis && (
              <div
                className="w-1 h-1 rounded-full bg-zinc-600 opacity-0 group-hover:opacity-100 
